...
.PHONY: help
help:
	@echo "Makefile for watchcode"
	@echo
	@echo "Usage: "
	@echo "		make build		- Build a Docker Image for watchcode backend"
	@echo "						  excuted automatically before run-"
	@echo
	@echo "		make run		- run watchcode backend and dependent services"
	...
	@echo "		make requirements - run pip install -r requirements/base.txt
	@echo
...
run-dev:stop clean
	$(docker-compose) run \
		--name watchcode \
		-e DJANGO_SETTINGS_MODULE=watchcode.settings.dev \
		-e version=${version} \
		-v `pwd`:/watchcode\
		-p 8002:8002 \
		backend dev
...
test-dev: stop build clean
	$(docker-compose) run \
		-e DJANGO_SETTINGS_MODULE=watchcode.settings.test \
		-v `pwd`:/watchcode\
		-p 8002:8002 \
		--name house-sell \
		backend test
...
stop:
	@$(docker-compose) stop
	@(docker ps -q -f 'name=watchcode*' | xargs docker stop 2>&1) >/dev/null || true
	@echo "Containers stopped."

.PHONY: clean
clean:
	@$(docker-compose) rm -f
	@(docker ps -q -f 'name=watchcode*' | xargs docker rm 2>&1) >/dev/null || true
	docker rm -fv watchcode || true
	@echo "Containers removed."

.PHONY: migration
migration:
	docker exec -ti watchcode python manage.py makemigrations \
	--name $(name) $(app)
	
.PHONY: migrate
migrate:
	docker exec -ti watchcode python manage.py migrate

.PHONY: createsuperuser
createsuperuser:
	docker exec -ti watchcode python manage.py createsuperuser
